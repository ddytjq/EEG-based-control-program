...
ArrayList<String> result = new ArrayList<String>(); // real-time alpha data //<>//
String line;
String[] pieces;
int j = 0;
...
float stepSize = 2.2; // random size

float distortionFactor = 1;
float initRadius = 60; // small shape
float initRadius2 = 105; // large shape

float centerX, centerY;

float[] x = new float[formResolution]; // small x
float[] y = new float[formResolution]; // small y
float[] a = new float[formResolution]; // large x
float[] b = new float[formResolution]; // large y
...
void setup() {
  ...
  // File i/o
  reader = createReader("/Users/kimyosub/Documents/Processing/File_io/tnals.txt");
  try {
  ...
  // impact Shape size
  for (int i = 0; i < formResolution; i++) {
    a[i] = cos(angle*i) * (initRadius2);
    b[i] = sin(angle*i) * (initRadius2);
  }
...
void draw() {
  ...
  for (int i=0; i<formResolution; i++) {
    ...
    a[i] += random(-stepSize, stepSize);
    b[i] += random(-stepSize, stepSize); 
  }
...
void drawShape() {
  ...
  for (int i=0; i<formResolution; i++) {
    print(float(result.get(j)), " ");
    if (float(result.get(j))<=30) {
      if (14<=float(result.get(j))) {
        curveVertex(x[0]+centerX, y[0]+centerY);
        ellipse(x[0]+centerX, y[0]+centerY, 5, 5);
        curveVertex(x[1]+centerX, y[1]+centerY);
        ellipse(x[1]+centerX, y[1]+centerY, 5, 5);
        curveVertex(x[2]+centerX, y[2]+centerY);
        ellipse(x[2]+centerX, y[2]+centerY, 5, 5);
        curveVertex(x[3]+centerX, y[3]+centerY);
        ellipse(x[3]+centerX, y[3]+centerY, 5, 5);
        curveVertex(x[4]+centerX, y[4]+centerY);
        ellipse(x[4]+centerX, y[4]+centerY, 5, 5);
        curveVertex(x[5]+centerX, y[5]+centerY);
        ellipse(x[5]+centerX, y[5]+centerY, 5, 5);
        // impact beta
        curveVertex(a[6]+centerX, b[6]+centerY);
        ellipse(a[6]+centerX, b[6]+centerY, 5, 5);
        //        
        j++;
      } else if (12<=float(result.get(j)) && 13.999>=float(result.get(j))) {
        // impact fast alpha
        curveVertex(a[0]+centerX, b[0]+centerY);
        ellipse(a[0]+centerX, b[0]+centerY, 5, 5);
        //
        curveVertex(x[1]+centerX, y[1]+centerY);
        ellipse(x[1]+centerX, y[1]+centerY, 5, 5);
        curveVertex(x[2]+centerX, y[2]+centerY);
        ...
        curveVertex(x[5]+centerX, y[5]+centerY);
        ellipse(x[5]+centerX, y[5]+centerY, 5, 5);                
        curveVertex(x[6]+centerX, y[6]+centerY);
        ellipse(x[6]+centerX, y[6]+centerY, 5, 5);
        j++;
      } else if (10<=float(result.get(j)) && 11.999>=float(result.get(j))) {
        curveVertex(x[0]+centerX, y[0]+centerY);
        ellipse(x[0]+centerX, y[0]+centerY, 5, 5);        
        // impact middle alpha
        curveVertex(a[1]+centerX, b[1]+centerY);
        ellipse(a[1]+centerX, b[1]+centerY, 5, 5);
        //
        curveVertex(x[2]+centerX, y[2]+centerY);
        ellipse(x[2]+centerX, y[2]+centerY, 5, 5);
        curveVertex(x[3]+centerX, y[3]+centerY);
        ...
        ellipse(x[6]+centerX, y[6]+centerY, 5, 5);
        j++;
      } else if (8<=float(result.get(j)) && 9.999>=float(result.get(j))) {
        curveVertex(x[0]+centerX, y[0]+centerY);
        ellipse(x[0]+centerX, y[0]+centerY, 5, 5);
        curveVertex(x[1]+centerX, y[1]+centerY);
        ellipse(x[1]+centerX, y[1]+centerY, 5, 5);        
        // impact slow alpha
        curveVertex(a[2]+centerX, b[2]+centerY);
        ellipse(a[2]+centerX, b[2]+centerY, 5, 5);
        //
        curveVertex(x[3]+centerX, y[3]+centerY);
        ellipse(x[3]+centerX, y[3]+centerY, 5, 5);
        curveVertex(x[4]+centerX, y[4]+centerY);
        ...
        ellipse(x[6]+centerX, y[6]+centerY, 5, 5);        
        j++;
      } else if (4<=float(result.get(j)) && 7.999>=float(result.get(j))) {
        curveVertex(x[0]+centerX, y[0]+centerY);
        ellipse(x[0]+centerX, y[0]+centerY, 5, 5);
        curveVertex(x[1]+centerX, y[1]+centerY);
        ellipse(x[1]+centerX, y[1]+centerY, 5, 5);
        ...
        ellipse(x[2]+centerX, y[2]+centerY, 5, 5);        
        // impact theta
        curveVertex(a[3]+centerX, b[3]+centerY);
        ellipse(a[3]+centerX, b[3]+centerY, 5, 5);
        //
        curveVertex(x[4]+centerX, y[4]+centerY);
        ellipse(x[4]+centerX, y[4]+centerY, 5, 5);
        curveVertex(x[5]+centerX, y[5]+centerY);
        ...
        ellipse(x[6]+centerX, y[6]+centerY, 5, 5);         
        j++;
      } else if (0.5<=float(result.get(j)) && 3.999>=float(result.get(j))) {
        curveVertex(x[0]+centerX, y[0]+centerY);
        ellipse(x[0]+centerX, y[0]+centerY, 5, 5);
        curveVertex(x[1]+centerX, y[1]+centerY);
        ellipse(x[1]+centerX, y[1]+centerY, 5, 5);
        ...
        ellipse(x[3]+centerX, y[3]+centerY, 5, 5);
        // impact delta
        curveVertex(a[4]+centerX, b[4]+centerY);
        ellipse(a[4]+centerX, b[4]+centerY, 5, 5);
        //
        curveVertex(x[5]+centerX, y[5]+centerY);
        ellipse(x[5]+centerX, y[5]+centerY, 5, 5);              
        curveVertex(x[6]+centerX, y[6]+centerY);
        ellipse(x[6]+centerX, y[6]+centerY, 5, 5);             
        j++;
      } else if ( 0 <= float(result.get(j)) && 0.4999 >= float(result.get(j))){
        j++;
      }
    } else if (30.0001<float(result.get(j)) && 50>=float(result.get(j))){
      curveVertex(x[0]+centerX, y[0]+centerY);
      ellipse(x[0]+centerX, y[0]+centerY, 5, 5);
      curveVertex(x[1]+centerX, y[1]+centerY);
      ellipse(x[1]+centerX, y[1]+centerY, 5, 5);
      ...
      ellipse(x[4]+centerX, y[4]+centerY, 5, 5);       
      // impact gamma
      curveVertex(a[5]+centerX, b[5]+centerY);
      ellipse(a[5]+centerX, b[5]+centerY, 5, 5);
      //
      curveVertex(x[6]+centerX, y[6]+centerY);
      ellipse(x[6]+centerX, y[6]+centerY, 5, 5);
      j++;
